...
class SchoolController extends GetxController {
  ...
  List<String> getTermList = ["Midterm","Final Term"];
    ...
    if (selectedDivision.value.isNotEmpty && selectedStandard.value.isNotEmpty) {
      ...
      if (resultData.docs.isNotEmpty) {
        ...
        for (int i = 0; i < resultModel.subjectNameList!.length; i++) {
          ...
          final mark = i < resultModel.totalMarksList!.length ? resultModel.totalMarksList![i] : '';
        ...
        getMarksList = resultModel.totalMarksList!;
  ...
  Future<void> submitResult({
    ...
    if (marksController.text.isEmpty && practicalMarksController.text.isEmpty) {
      Get.snackbar("Notice", "Please enter marks", backgroundColor: Colors.red, colorText: Colors.white);
      return;
    }
    ...
    try {
      ...
      final schoolData = ResultModel(
        ...
        schoolName: CurrentUserData.schoolName,
        subjectTotalMarks: marksController.text.isEmpty ? '0' : marksController.text,
        subjectPracticalMarks: practicalMarksController.text.isEmpty ? '0' : practicalMarksController.text,
  ...
  Future<void> getStudentMarks({required String subjectId, required String studentUid,required String div,required String std,required String subjectName}) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      marksController.text = resultModel.subjectTotalMarks ?? "0";
      practicalMarksController.text = resultModel.subjectPracticalMarks ?? "0";
    } else {
